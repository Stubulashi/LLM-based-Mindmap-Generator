...
class MarkdownReportRenderer:
    ...
    def _render_header(self, gold_map, gen_map, config_info, example_mode: bool = False) -> str:
        ...
        if gen_map and gen_map.metadata:
            lines.append(f"**Map / 导图**: {gen_map.metadata.get('name', 'N/A')}")
            lines.append(f"**Map ID**: {gen_map.metadata.get('map_id', 'N/A')}")
        ...
        lines.append(f"**Generated Nodes / 生成节点数**: {gen_map.node_count}")
        return '\n'.join(lines)
    ...
    def _render_summary(self, results: dict, inclusion_list, example_mode: bool = False) -> str:
        ...
        if results:
            ...
            score, detail = compute_composite_score(label_dict, hierarchy_dict, qa_dict)
            if score > 0:
                lines.append(
        ...
        dims = [
            ...
            ('qa', 'QA / 问答', 'qa_retention', 'QA Retention'),
            ('efficiency', 'Efficiency / 效率', 'wer', 'WER'),
            ('multilingual', 'Multilingual / 多语言', 'max_delta_recall', 'Max Δ Recall'),
            ('human_corr', 'Human Corr / 人工对齐', 'node_f1_readability_r', 'Pearson r'),
        ]
    ...
    def _render_qa_section(self, metrics: Optional[dict]) -> str:
        if not metrics:
            return "---\n## 3. Downstream QA / 下游问答测试\n\n*Not executed / 未执行（requires preset questions and LLM API）*"
        lines = [
            "---",
            "## ❓ 3. Downstream QA / 下游问答测试",
            "",
            ...
            "|---|---|---|---|",
            f"| Control (Full) / 对照组 | {metrics.get('control_accuracy', 0):.3f} | — | baseline |",
            f"| Experiment (Map) / 实验组 | {metrics.get('experiment_accuracy', 0):.3f} | — | {metrics.get('qa_retention', 0):.3f} |",
            "",
            f"**QA Retention**: {metrics.get('qa_retention', 0):.3f}",
            f"**Token Reduction / Token 缩减**: {metrics.get('token_reduction', 0):.1%}",
        ]
        if metrics.get('error'):
    ...
    def _render_correlation_section(self, metrics: Optional[dict]) -> str:
        if not metrics:
            return "---\n## 6. Human Alignment / 人工对齐效度\n\n*Not executed / 未执行（requires human scoring data）*"
        lines = [
            "---",
            "## 👤 6. Human Alignment / 人工对齐效度",
            "",
    ...
    def _render_diagnostics(self, results: dict) -> str:
        ...
        qa = results.get('qa')
        if qa:
            qar = qa.get('qa_retention')
            if qar is not None and qar < 0.75:
                suggestions.append(
                    "> **⚠️ QA Retention Needs Improvement ({:.3f}) / QA 保持率需改进**. ".format(qar)
